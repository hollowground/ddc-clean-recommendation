...
# Function to compute optimal time between cleanings
def compute_target_time(row):
    """Function will create a target cleaning cadence in hours based on the data provided."""
...
# Apply the function to each row and add new field
for row in data:
    row["target_time_between_cleaning"] = compute_target_time(row)
...
# Save to CSV
df.to_csv("cleaning_schedule_with_target.csv", index=False)

# Print confirmation
print("CSV file 'cleaning_schedule_with_target.csv' has been created.")
